...
class ComputeSimilarity:
    ...
    def compute_similarity(self, method, block_size=100):
        ...
        if method == "user":
            ...
            sumOfSquared = np.sqrt(sumOfSquared) # edit
        ...
        while start_block < end_local:
            ...
            if method == "user":
                ...
                data = self.dataMatrix[:, start_block:end_block] # user_num * blocksize
            data = data.toarray()
            ...
            if method == "user":
                ...
                this_block_weights = self.dataMatrix.T.dot(data) # (item_num, user_num) * (user_num, blocksize) = This block's item & all item's similarity.

            for index_in_block in range(this_block_size):
                this_line_weights = this_block_weights[:, index_in_block] # cur_item dot all_items
                ...
                if self.normalize:
                    denominator = (
                        sumOfSquared[Index] * sumOfSquared + self.shrink + 1e-6 # cur_sum * all item's sum, a vector of (item_num)
                    )
...
class ItemKNN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        # edit
        def check_matrix(matrix, id_dict):
            with open("./check_matrix.txt", "w") as f:
                userid_arr = id_dict['user_id']
                itemid_arr = id_dict['item_id']
                for i in range(matrix.shape[0]):
                    for j in range(matrix.shape[1]):
                        if abs(matrix[i][j] - 1.0) < 1e-6:
                            f.write("{}|{}\n".format(userid_arr[i], itemid_arr[j]))
            print("done")
        # edit end
        ...
        check_matrix(self.interaction_matrix.toarray(), dataset.field2id_token)
