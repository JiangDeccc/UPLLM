...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        saved_model_file = "{}-{}-{}-{}.pth".format(self.config["dataset_type"], self.config["model"], get_local_time(), md5) # edit
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        #     user = interaction_list[i]['user_id']
        #     item = interaction_list[i]['item_id']
        #     label = interaction_list[i]['rating']
        #     print(user.device, item.device, label.device)
        #     tm.sleep(0.02)
        ...
        for interaction in interaction_list:
            ...
            # interaction = interaction.to(self.device)
            # return 0.0
            interaction = interaction_list[batch_idx]
    ...
    def _train_epoch_preprocess(self, train_data, interaction_list, epoch_idx, loss_func=None, show_progress=False):
        r"""Train the model in an epoch
    ...
    def _valid_epoch_preprocess(self, valid_data, evaluation_list, batched_data_list, show_progress=False):
        r"""Valid the model with valid data
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result = self.evaluate_preprocess(
            valid_data, evaluation_list, batched_data_list, load_best_model=False, show_progress=show_progress
        )
        # valid_score = 0.0
        valid_score = calculate_valid_score(valid_result, self.valid_metric)
        return valid_score, valid_result

    def _valid_epoch_labeled_ranking_preprocess(self, evaluation_list):
        r"""Evaluate fixed labeled candidates as per-user ranking data.

        RecBole's labeled mode is value-based and does not provide positive_u/positive_i.
        For fixed candidate sets, compute ranking metrics directly from the labeled
        interaction rows instead of using the ranking collector.
        """
        self.model.eval()
        user_results = {}

        with torch.no_grad():
            for interaction in evaluation_list:
                scores = self.model.predict(interaction)
                self._append_labeled_ranking_rows(user_results, interaction, scores)

        result = self._labeled_ranking_result(user_results)

        valid_score = calculate_valid_score(result, self.valid_metric)
        return valid_score, result

    def _append_labeled_ranking_rows(self, user_results, interaction, scores):
        users = interaction[self.config["USER_ID_FIELD"]].detach().cpu()
        labels = interaction[self.config["LABEL_FIELD"]].detach().cpu()
        scores = scores.detach().cpu()

        # Raw ratings use >= threshold as positive; thresholded labels use > 0.
        if labels.numel() > 0 and labels.max().item() > 1:
            threshold = self.config.final_config_dict.get("threshold", {}).get(
                self.config["LABEL_FIELD"], 0
            )
            positives = labels >= threshold
        else:
            positives = labels > 0

        for user, is_pos, score in zip(users.tolist(), positives.tolist(), scores.tolist()):
            user_results.setdefault(user, []).append((int(is_pos), float(score)))

    def _labeled_ranking_result(self, user_results):
        topk = self.config["topk"]
        metrics = [metric.lower() for metric in self.config["metrics"]]
        result = OrderedDict()
        ndcg_sum = {k: 0.0 for k in topk}
        mrr_sum = {k: 0.0 for k in topk}
        valid_user_num = 0

        for rows in user_results.values():
            valid_user_num += 1
            ...
            for k in topk:
                ...
                reciprocal_sum = 0.0
                hit_num = 0
                for rank, (label, _) in enumerate(top_rows, start=1):
                    if label:
                        hit_num += 1
                        dcg += 1.0 / np.log2(rank + 1)
                        reciprocal_sum += 1.0 / rank

                # Match test2_100_base.py:get_ndcg_mrr: IDCG is based on
                # positives found in the top-k slice, not all user positives.
                idcg = sum(1.0 / np.log2(rank + 1) for rank in range(1, hit_num + 1))
                if hit_num > 0 and idcg > 0:
                    ndcg_sum[k] += dcg / idcg
                    mrr_sum[k] += reciprocal_sum / hit_num

        denominator = max(valid_user_num, 1)
        for k in topk:
            if "ndcg" in metrics:
                result[f"ndcg@{k}"] = ndcg_sum[k] / denominator
            if "mrr" in metrics:
                result[f"mrr@{k}"] = mrr_sum[k] / denominator
        return result
    ...
    def _save_checkpoint2(self, epoch, verbose=True, **kwargs):
        r"""Store the model parameters information and training information.
    ...
    def _save_checkpoint3(self, epoch, verbose=True, **kwargs):
        r"""Store the model parameters information and training information.
        ...
        if not self.config["single_spec"] and self.config["local_rank"] != 0:
            return
        saved_model_file = kwargs.pop("saved_model_file", self.saved_model_file)
        # print("****** save: {}".format(state['epoch']))
        torch.save(self.best_state, saved_model_file, pickle_protocol=4)
        if verbose:
            self.logger.info(
                set_color("Saving current", "blue") + f": {saved_model_file}"
            )
    ...
    def fit(
        ...
        # print(type(interaction_list[0]), type(evaluation_list[0]))
        # tm.sleep(1000)
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # )
            train_loss = self._train_epoch_preprocess(
                train_data, interaction_list, epoch_idx, show_progress=show_progress
            )
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # )
                valid_score, valid_result = self._valid_epoch_preprocess(
                    valid_data, evaluation_list, show_progress=show_progress
                )
                # valid_score = 0.0
                # valid_result = OrderedDict()
                # valid_result['auc'] = 0.0
                ...
                if update_flag:
                    if saved:
                        # self._save_checkpoint(epoch_idx, verbose=verbose)
                        self._save_checkpoint2(epoch_idx, verbose=verbose)
        ...
        self._save_checkpoint3(epoch_idx, verbose=verbose) # edit
    ...
    def fit_preprocess(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # )
                if (
                    self.config["eval_args"]["mode"] == "labeled"
                    and self.config["eval_type"] == EvaluatorType.RANKING
                ):
                    valid_score, valid_result = self._valid_epoch_labeled_ranking_preprocess(
                        evaluation_list
                    )
                else:
                    valid_score, valid_result = self._valid_epoch_preprocess(
                        valid_data, evaluation_list, batched_data_list, show_progress=show_progress
                    )
                # valid_score = 0.0
                # valid_result = OrderedDict()
                # valid_result['auc'] = 0.0
    ...
    def _neg_sample_batch_eval_preprocess(self, batched_data, interaction2):
        interaction, row_idx, positive_u, positive_i = batched_data
        batch_size = interaction.length
        # print(batch_size, self.test_batch_size)
        # tm.sleep(1000)
        if batch_size <= self.test_batch_size:
            # origin_scores = self.model.predict(interaction.to(self.device))
            # print(interaction2['user_id'].device)
            # tm.sleep(1000)
            origin_scores = self.model.predict(interaction2)
            # origin_scores = interaction2['rating']
        else:
            origin_scores = self._spilt_predict(interaction, batch_size)
    ...
    def _evaluate_labeled_ranking(self, eval_data, show_progress=False, filename=None):
        iter_data = (
            tqdm(
                eval_data,
                total=len(eval_data),
        ...
        folder = None
        if filename is not None and filename != "train":
            folder = self.saved_model_file.split(".")[0]
            if not os.path.exists(folder):
                os.mkdir(folder)
            if not os.path.exists("{}/{}".format(folder, filename)):
                os.mkdir("{}/{}".format(folder, filename))

        user_results = {}
        with tqdm(total=len(iter_data), desc="Evaluating...") as pbar:
            for batch_idx, batched_data in enumerate(iter_data):
                interaction = batched_data[0]
                model_interaction = interaction.to(self.device)
                scores = self.model.predict(model_interaction)
                self._append_labeled_ranking_rows(user_results, interaction, scores)

                if filename is not None and filename != "train":
                    grouped_data = (interaction, scores.detach().cpu(), None, None)
                    with open("{}/{}/batched_data_{}.pkl".format(folder, filename, batch_idx), "wb") as f:
    ...
    @torch.no_grad()
    def evaluate(
        self, eval_data, load_best_model=True, model_file=None, show_progress=False, filename=None
    ):
        r"""Evaluate the model based on the eval data.
        ...
        if (
            ...
            return self._evaluate_labeled_ranking(
                eval_data, show_progress=show_progress, filename=filename
            )
        ...
        if filename is not None and filename != "train": # edit
            folder = self.saved_model_file.split(".")[0]
            if not os.path.exists(folder):
                os.mkdir(folder) # edit end
        ...
        if filename == "train":
            ...
            total_scores = []
            # length = 0
            # with tqdm(total=len(eval_data)) as pbar2:
            #     for batch_idx, batched_data in enumerate(iter_data):
            #         length += batched_data[0].length
            #         pbar2.update()
            # total_userid = torch.zeros(length)
            # total_itemid = torch.zeros(length)
            # total_rating = torch.zeros(length)
            # total_scores = torch.zeros(length)
            # startindex = 0
        ...
        with tqdm(total=len(iter_data), desc="Evaluating...") as pbar:
            for batch_idx, batched_data in enumerate(iter_data):
                ...
                #     print(batched_data[i], type(batched_data[i]), batched_data[i].shape)
                    
                # tm.sleep(1000)
            # import pdb; pdb.set_trace()
            # for batch_idx, batched_data in eval_data:
                ...
                if filename is not None and filename != "train": # edit
                    grouped_data = (interaction, scores, positive_u, positive_i)
                    if not os.path.exists("{}/{}".format(folder, filename)):
                        os.mkdir("{}/{}".format(folder, filename))
                    with open("{}/{}/batched_data_{}.pkl".format(folder, filename, batch_idx), "wb") as f:
                        pickle.dump(grouped_data, f)
                ...
                if filename == "train":
                    ...
                    cur_rating = interaction['rating_ori'].tolist()
                    # cur_userid = interaction['user_id']
                    # cur_itemid = interaction['item_id']
                    # cur_rating = interaction['rating_ori']
                    # print(interaction['rating'].tolist())
                    # print(cur_rating)
                    # tm.sleep(1000)
                    cur_scores = scores.tolist()
                    # total_userid += cur_userid
                    # total_itemid += cur_itemid
                    # total_rating += cur_rating
                    # total_scores += cur_scores
                    total_userid.append(cur_userid)
                    total_itemid.append(cur_itemid)
                    total_rating.append(cur_rating)
                    total_scores.append(cur_scores)
                    # total_userid = []
                    # total_itemid = []
                    # total_rating = []
                    # total_scores = []
                    # total_userid[startindex:startindex + batched_data[0].length] = cur_userid
                    # total_itemid[startindex:startindex + batched_data[0].length] = cur_itemid
                    # total_rating[startindex:startindex + batched_data[0].length] = cur_rating
                    # total_scores[startindex:startindex + batched_data[0].length] = cur_scores
                    # startindex += batched_data[0].length
                # edit end
        ...
        if filename != "train":
            ...
            #     if isinstance(self.logger.handlers[i], FileHandler):
            #         logger_path = self.logger.handlers[i].baseFilename
            #         break
            # folder_path = logger_path.split(".")[0]
            # if not os.path.exists(folder_path):
            #     os.mkdir(folder_path)
            # with open("{}/train_full.pkl".format(folder_path), "wb") as f:
            #     pickle.dump((total_userid, total_itemid, total_scores), f)
            print("Merging userid...")
            total_userid = list(chain(*total_userid))
            print("Merging itemid...")
            total_itemid = list(chain(*total_itemid))
            print("Merging rating...")
            total_rating = list(chain(*total_rating))
            print("Merging scores...")
            total_scores = list(chain(*total_scores))
            print("Merging done!")
            tuple_result = (total_userid, total_itemid, total_rating, total_scores)
            return tuple_result
    
    @torch.no_grad()
    def evaluate_preprocess(
        self, eval_data, evaluation_list, batched_data_list, load_best_model=True, model_file=None, show_progress=False, filename=None
    ):
        r"""Evaluate the model based on the eval data.
        ...
        if isinstance(eval_data, FullSortEvalDataLoader):
            ...
            eval_func = self._neg_sample_batch_eval_preprocess
        if self.config["eval_type"] == EvaluatorType.RANKING:
            self.tot_item_num = eval_data._dataset.item_num
        ...
        if filename is not None:
            pbar = tqdm(total=len(batched_data_list), desc="Processing batched data list...")
        
        
        for batch_idx, batched_data in enumerate(batched_data_list):
        # for batch_idx in range(len(batched_data_list)):
        #     batched_data = batched_data_list[batch_idx]
            # print(batched_data)
            # for i in range(1, len(batched_data)):
            #     print(batched_data[i], type(batched_data[i]), batched_data[i].shape)
        ...
        # for batch_idx, batched_data in eval_data:
            num_sample += len(batched_data)
            interaction, scores, positive_u, positive_i = eval_func(batched_data, evaluation_list[batch_idx])
            # interaction, _, positive_u, positive_i = batched_data
            # scores = evaluation_list[batch_idx]['rating']
            # print(interaction)
            # tm.sleep(1000)
            # print(interaction['user_id'])
            
            # if filename is not None: # edit deprecated
            #     grouped_data = (interaction, scores, positive_u, positive_i)
            #     if not os.path.exists("{}/{}".format(folder, filename)):
            #         os.mkdir("{}/{}".format(folder, filename))
            #     with open("{}/{}/batched_data_{}.pkl".format(folder, filename, batch_idx), "wb") as f:
            #         pickle.dump(grouped_data, f) # edit end
            if filename is not None:
                # print(interaction['user_id'].shape, interaction['item_id'].shape, scores.shape)
                # tm.sleep(1000)
                grouped_data_list['user_id'].append(interaction['user_id'])
                grouped_data_list['item_id'].append(interaction['item_id'])
                grouped_data_list['scores'].append(scores)
                pbar.update()
            ...
            self.eval_collector.eval_batch_collect_preprocess(
                scores, evaluation_list[batch_idx], positive_u, positive_i
            )
        # print("begin sleep {}".format(batch_idx))
        # tm.sleep(1000) # edit
            # import pdb; pdb.set_trace()
        # print(scores.shape) # edit
        self.eval_collector.model_collect(self.model)
        struct = self.eval_collector.get_data_struct()
        # rec.score, data.label
        result = self.evaluator.evaluate(struct)
        if not self.config["single_spec"]:
            result = self._map_reduce(result, num_sample)
        self.wandblogger.log_eval_metrics(result, head="eval")
        ...
        if filename is None:
            ...
            for key in grouped_data_list.keys():
                grouped_data_list[key] = torch.cat(grouped_data_list[key], dim=0)
                grouped_data_list[key] = grouped_data_list[key].detach().cpu().tolist()
            return result, grouped_data_list
